<template>
  <transition name='fade'>
    <div class='anoahimdiv' v-bind:class='{"phone":vthis&&vthis.rootdata.screenflag==2,"pad":vthis&&vthis.rootdata.screenflag==1&&vthis.rootdata.bookflag==0,"book_pad":vthis&&vthis.rootdata.screenflag==1&&vthis.rootdata.bookflag==1}' v-show='show'>
      <div class='aimtooldiv'>
        <div v-if='type==0' class='aimresultdiv'>
          <div class='pinyin'>{{pinyinstr}}</div>
          <div class='result'>
            <ol>
              <li v-if='key<10' v-for='(value,key) in hanziarray' :key='key' :data-idx='(key+1)' v-on:click.stop='choicehz(value)'>{{value}}</li>
            </ol>
          </div>
        </div>
        <div class='aimclosediv' v-on:click='closeclick'>关闭{{type}}</div>
      </div>
      <div class='aimbigbndiv'></div>
      <div class='aimbndiv'>
        <div v-for='(value,key) in (numflag==1&&type==0)?keypadjson[type].numsrc:keypadjson[type].src' :key='key' class='aimbnrowdiv'>
          <div v-for='(value1,key1) in value' :key='key1' class='signnumdiv' v-bind:class='[value1.class]' v-on:click='bnclick($event,(capflag==1&&type==0&&value1.value.length==1)?value1.value.toUpperCase():value1.value)'>{{(capflag==1&&type==0&&value1.value.length==1)?value1.value.toUpperCase():value1.value}}</div>
        </div>
      </div>
    </div>
  </transition>
...
export default {
  ...
  created: function() {
    ...
    //console.log(this.pyinputjson["a"]);
  },
  methods: {
    //拼音输入选中汉字
        ...
        for (let j = 0; j < this.inputarray[i].length; j++) {
          if (this.inputarray[i][j].name == "cursor") {
            ...
            if (
              ...
              this.deletefraccur(this.inputarray[i][j].value.up);
              this.deletefraccur(this.inputarray[i][j].value.down);
            }
          }
        }
      }
...
*/
    ...
    closeclick: function(event) {
      ...
      this.vthis.$nextTick(() => {
        ...
        if (qtimain.scrollTop + qtimain.offsetHeight >= qtimain.scrollHeight) {
